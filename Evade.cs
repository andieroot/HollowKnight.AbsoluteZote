        ...
        private void UpdateFSMEvade(PlayMakerFSM fsm)
        {
            ...
            void evade()
            {
                ...
                foreach (var rootGameObject in rootGameObjects)
                {
                    if (rootGameObject.name == "Fireball2 Spiral(Clone)")
                    {
                        ...
                        if (fireballPositon.y - myPosition.y < 2)
                        {
                            ...
                            if (System.Math.Abs(xDiff) <= 11 && System.Math.Sign(fireballVelocity.x) == System.Math.Sign(xDiff))
                            {
                                fsm.AccessFloatVariable("evadeVelocityX").Value = System.Math.Sign(fireballVelocity.x) * 5;
                ...
                if (spells.transform.Find("Scr Heads 2").gameObject.activeSelf)
                {
                    ...
                    if (System.Math.Abs(myPosition.x - heroPositon.x) <= 5)
                    {
                        fsm.AccessFloatVariable("evadeVelocityX").Value = System.Math.Sign(myPosition.x - heroPositon.x) * 20;
